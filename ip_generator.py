...
def main(ip_range, json_file):
    for ip in IPv4Network(ip_range).hosts():
        ...
        valid_html_page = check_html_page(ip_str)
        print(
            f"IP: {ip_str} - Response Rate: {response_rate * 100}% - Valid HTML Page: {valid_html_page}")

        if response_rate > 0.75 and valid_html_page:
